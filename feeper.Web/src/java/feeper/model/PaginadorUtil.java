...
public class PaginadorUtil<T> {
    ...
    public List<T> Execute(Class objClass, Model model, String query, Object[] params, int currentPage, int pageSize, String sortField, String sortDirection) {
        return Execute(objClass, model, query, params, null, currentPage, pageSize, sortField, sortDirection);
    }

    public List<T> Execute(Class objClass, Model model, String query, int currentPage, int pageSize, String sortField, String sortDirection) {
        return Execute(objClass, model, query, null, null, currentPage, pageSize, sortField, sortDirection);
    }

    public List<T> Execute(Model model, String query, Object[] params, ScalarResult[] scalarResult, int currentPage, int pageSize, String sortField, String sortDirection) {
        return Execute(null, model, query, params, scalarResult, currentPage, pageSize, sortField, sortDirection);
    }

    public List<T> Execute(Model model, String query, ScalarResult[] scalarResult, int currentPage, int pageSize, String sortField, String sortDirection) {
        return Execute(null, model, query, null, scalarResult, currentPage, pageSize, sortField, sortDirection);
    }

    private List<T> Execute(Class objClass, Model model, String query, Object[] params, ScalarResult[] scalarResult, int currentPage, int pageSize, String sortField, String sortDirection) {
    ...
    private List ExecutePagedQuery(Class objClass, String query, Object[] params, ScalarResult[] scalarResult, int pageSize, int currentPage, IntegerResult count) {
        ...
        Transaction transaction = repo.currentSession().beginTransaction();
        try {
            ...
            SQLQuery exec = repo.currentSession().createSQLQuery(command);

            if (objClass != null) {
                exec.addEntity(objClass);
            } else if (scalarResult != null) {
            ...
            List list = exec.list();
            count.setResult(((BigInteger) repo.currentSession().createSQLQuery(commandCount).uniqueResult()).intValue());
    ...
    private String GetPages(int currentPage, int totalPages) {
        ...
        if (totalPages > 1) {
            ...
            if (currentPage - previous < maxPagesShow) {
                next += maxPagesShow - (currentPage - previous);
            }
            if (next - currentPage < maxPagesShow) {
                previous -= maxPagesShow - (next - currentPage);
            }
