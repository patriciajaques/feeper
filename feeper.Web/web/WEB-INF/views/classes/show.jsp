    ...
    <jsp:body>
        ...
        <div class="panel panel-success shadow" id="panel-markedannotations" style="width:400px; display:none; position:fixed; top:120px; left:0px; z-index:100">
            ...
            <form role="form" action="<c:url value='/'/>exercicios/saveannotation" id="frmAnotacao" method="POST">
                <div class="panel-body" id="pnlAnotacoes" style="height: 300px; overflow-y: auto">
                    <textarea class="form-control" rows="12" id="anotacaoExercicioClasse" name="anotacaoExercicioClasse"></textarea>
                </div>
                <div class="panel-footer">
                    <input type="hidden" id="hdnAnotacaoIdExercicioClasse" name="hdnAnotacaoIdExercicioClasse">
        ...
        <script type="text/javascript">
            ...
            $(function () {
                ...
                var ajaxFormOptions2 = {
                    ...
                    success: function (data) {
                        $.fancybox.hideLoading();
                        $("#anotacaoExercicioClasse").val(data[1]);
            ...
            function exibeQuadroDuvida(row)
            {
                ...
                $("#hdnQuestaoIdExercicioClasse").val(${idExercicioClasse});
                $("#hdnQuestaoIdExercicio").val(${idExercicio});
                $("#hdnQuestaoLinha").val(row);

                $("#panel-markedquestions .panel-body").load("<c:url value='/'/>exercicios/showquestion/${idExercicio}/${idExercicioClasse}/" + row, function () {
                    $("#panel-markedquestions .panel-body").animate({scrollTop: $("#panel-markedquestions .panel-body")[0].scrollHeight}, 1000);
                });
            }
            ...
            function exibeQuadroAnotacao(row)
            {
                ...
                $("#hdnAnotacaoIdExercicioClasse").val(${idExercicioClasse});
                $("#hdnAnotacaoIdExercicio").val(${idExercicio});
                $("#hdnAnotacaoLinha").val(row);

                $.get("<c:url value='/'/>exercicios/showannotation/${idExercicio}/${idExercicioClasse}/" + row, function (data) {
                    $("#anotacaoExercicioClasse").val(data[1]).focus();
                    $("#panel-markedannotations .panel-body").animate({scrollTop: $("#panel-markedannotations .panel-body")[0].scrollHeight}, 1000);
                });
            }
        </script>
